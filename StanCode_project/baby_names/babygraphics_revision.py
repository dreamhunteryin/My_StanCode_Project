...
def draw_names(canvas, name_data, lookup_names):
    ...
    for name in lookup_names:
        dic_yr_rank = name_data[name]
        old_x = get_x_coordinate(c_w, n)

        if year0 not in dic_yr_rank:
            old_y = c_h - margin * 2
            # canvas.create_text(old_x + TEXT_DX, old_y, text=f'{name}, *', anchor=tkinter.NW, font='times 10')
            # 上一行不用，因為你其實下面for loop裡加text, 都是加在上一個年份的位置
        else:
            old_rank = dic_yr_rank[year0]
            old_y = get_y_coordinate(c_h, int(old_rank))
        for year in YEARS:
            year_x = get_x_coordinate(c_w, n)
            if str(year) not in dic_yr_rank:
                rank_y = c_h - margin * 2
                # canvas.create_text(year_x + TEXT_DX, old_y, text=f'{name}, *', anchor=tkinter.NW, font='times 10')
                # 上面可省略
            # elif int(dic_yr_rank[f'{year}']) > 1000 :   # 這個elif區塊都可以省略
            #     rank_y = c_h - margin * 2
            #     canvas.create_text(year_x + TEXT_DX, old_y, text=f'{name}, *', anchor=tkinter.NW, font='times 10')
            ...
            if old_y == c_h - margin * 2:  # 縮排應該往前
                # text位置應該是old_x, old_y，並使用old_rank
                # canvas.create_text(year_x + TEXT_DX, rank_y, text=f'{name}{rank}', anchor=tkinter.NW, font='times 10')
                canvas.create_text(old_x + TEXT_DX, old_y, text=f'{name}, *', anchor=tkinter.NW,
                                   font='times 10')
            else:
                canvas.create_text(old_x + TEXT_DX, old_y, text=f'{name}{old_rank}', anchor=tkinter.NW,
                                   font='times 10')
